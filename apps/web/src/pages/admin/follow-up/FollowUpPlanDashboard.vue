  ...
  <div class="page-container">
    ...
    <div class="card-grid">
      ...
      <div class="stat-card">
        ...
        <div class="stat-value" style="color:#e6a23c">{{ stats.draft || 0 }}</div>
      </div>
      <div class="stat-card">
        <div class="stat-label">执行中</div>
        <div class="stat-value" style="color:#409eff">{{ stats.executing || 0 }}</div>
      </div>
      <div class="stat-card">
    ...
    <div class="table-card">
      ...
      <el-table :data="plans" stripe v-loading="loading" style="margin-top:16px">
        ...
        <el-table-column label="AI摘要" min-width="200" show-overflow-tooltip>
          <template #default="{ row }">{{ row.aiSummary?.substring(0, 60) || '-' }}...</template>
        </el-table-column>
        ...
        <template #empty>
          <div style="padding:40px; text-align:center">
            ...
            <p style="color:#c0c4cc; font-size:13px; margin-top:8px">当咨询师创建会话并完成 AI 分析后，系统会自动生成跟进策略草稿</p>
          </div>
        </template>
      </el-table>
    </div>
...
async function fetchData() {
  ...
  const [plansRes, statsRes]: any[] = await Promise.all([
    request.get('/follow-up-plans', { params }),
    request.get('/follow-up-plans/stats'),
  ]);
...
function goDetail(rowOrId: any) {
  ...
  if (id) {
    router.push(`/admin/follow-up-plans/${id}`);
  }
}
